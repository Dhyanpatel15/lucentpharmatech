...
app.get("/api/health", (req, res) => {
  res.status(200).json({ status: "ok", message: "Lucent Pharmatech SMTP Mailer API is running" });
});
...
app.post("/api/contact", async (req, res) => {
  try {
    ...
    if (!host || !user || !pass) {
      console.warn("SMTP credentials not fully configured in .env. Logging request payload for testing:");
      console.log({ name, email, phone, subject, message });

      return res.status(200).json({
        success: true,
        message: "Request received (SMTP is in test/mock mode until credentials are provided in .env)."
      });
    }
    ...
    const transporter = nodemailer.createTransport({
      ...
      auth: { user, pass },
      tls: {
        // Do not fail on invalid certificates (useful for custom domain mail servers)
    ...
    return res.status(200).json({
      success: true,
      message: "Your request has been sent successfully to panchalshravand@gmail.com and shravanog01@gmail.com."
    });
...
app.listen(PORT, () => {
  console.log(`Lucent Pharmatech SMTP Mailer API running on port ${PORT}`);
});
